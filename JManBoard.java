...
public class JManBoard {
    ...
    private Block block;      // The Block on this game board. 
    private Pillar pillar;      // The Pillars are on this game board.
    private Walker walker;      // The walkers are on the game board.  
    ...
    public void placePiece(int t, int x, int y){
        if (isEmpty(x,y) && t == 1) {
            jMan = new JMan(x,y,JManApp.rand(0,2));
            board[x][y] = jMan;
        }
        else if (isEmpty(x,y) && t == 0) {
            Block block = new Block(x,y,Color.WHITE);
            board [x][y] = block;
        }
        else if (isEmpty(x,y) && t == 2) {
            Pillar pillar = new Pillar(x,y,JManApp.rand(0,2));
            board [x][y] = pillar;
      }
        else if (isEmpty(x,y) && t == 3) {
            Walker walker = new Walker(x,y,JManApp.rand(0,2));
            board [x][y] = walker;
        }
    }
}